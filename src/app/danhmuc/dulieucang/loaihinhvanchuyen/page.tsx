...
import rawData from "@/data/loaihinhvanchuyen.json";
...
const STORAGE_KEY = "loaihinhvc-data";

export default function LoaiHinhVanChuyenPage() {
  ...
  const columns: TableProps<Row>["columns"] = useMemo(
    () => [
      { title: "Mã loại hình", dataIndex: "maLH", with: 100, align: "center",
        render: (val, r) =>
          r.editable
            ? ( <Input
                  bordered={false}
                  value={val}
                  onChange={e => updateCell(r.id, "maLH", e.target.value)}
                  style={{ textAlign: "center", width: "100%", height: "100%" }}
                /> )
              : val
          },
          { title: "Tên loại hình", dataIndex: "tenLH", with: 100, align: "center",
            render: (val, r) =>
              r.editable
                ? ( <Input
                  bordered={false}
                  value={val}
                  onChange={e => updateCell(r.id, "tenLH", e.target.value)}
                  style={{ textAlign: "center", width: "100%", height: "100%" }}
                /> )
              : val
          },
  ...
  return (
    <section id="loai-hinh-van-chuyen" className="mb-8 scroll-mt-[84px] space-y-4">
      <DataTable<Row>
