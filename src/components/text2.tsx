...
function SidebarItem({
  ...
  const isDashboard =
    ...
    (typeof node.label === "string" &&
      node.label.toLowerCase() === "dashboard");
  ...
  const activeText =
    !isDashboard && (isSelected || (hasChildren && isOpen))
      ? "text-[#9cd4ea]"
      : "";
  const activeBg =
    !isDashboard && (isSelected || (hasChildren && isOpen)) ? selectedBg : "";
  ...
  return (
    <li>
      ...
      >
        <i
          className={`las la-${node.icon ?? "box"} text-xl text-current`}
          aria-hidden
        />
        {!collapsed && (
          <>
            ...
            {hasChildren && (
              <i
                className={`las la-chevron-right transition-transform duration-200 ${
                  isOpen ? "rotate-90" : ""
                }`}
              />
            )}
      ...
      {hasChildren && (
        ...
        >
          <ul
            className={`overflow-hidden mt-1 space-y-1 transition-opacity duration-200 ease-out ${
